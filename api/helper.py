...
def generate_report_data(store):
    ...
    data = []
    print(last_one_hour_data, last_one_day_data, last_one_week_data)
...
def generate_csv_file( report, csv_data):
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with open(temp_file_path, "w", newline='') as csv_file:
            ...
            csv_writer.writerow(["store_id", "uptime_last_hour(in minutes)", "downtime_last_hour(in minutes)", "uptime_last_day(in hours)", "downtime_last_day(in hours)", "uptime_last_week(in hours)", "downtime_last_week(in hours)"])
...
def get_last_one_hour_data(store, utc_time, current_day, current_time):
    last_one_hour_data = {"uptime_last_hour(in minutes)" : 0 , "downtime_last_hour(in minutes)" : 0}
    ...
    if last_one_hour_logs:
        ...
        if last_one_hour_log_status == StoreStatus.ACTIVE:
            last_one_hour_data["uptime_last_hour(in minutes)"] = 60
        else:
            last_one_hour_data["downtime_last_hour(in minutes)"] = 60

    return last_one_hour_data
    

def get_last_one_day_data(store, utc_time, current_day, current_time):
    last_one_day_data = {"uptime_last_day(in hours)" : 0 , "downtime_last_day(in hours)" : 0}
    ...
    for log in last_one_day_logs:
        ...
        if log.status == StoreStatus.ACTIVE:
            last_one_day_data["uptime_last_day(in hours)"] += 1
        else:
            last_one_day_data["downtime_last_day(in hours)"] += 1
    return last_one_day_data

def get_last_one_week_data(store, utc_time, current_day, current_time):
    last_one_week_data = {"uptime_last_week(in hours)" : 0 , "downtime_last_week(in hours)" : 0}
    ...
    for log in last_one_week_logs:
        ...
        if not log_in_store_business_hours:
            continue
        if log.status == StoreStatus.ACTIVE:
            last_one_week_data["uptime_last_week(in hours)"] += 1
        else:
            last_one_week_data["downtime_last_week(in hours)"] += 1
